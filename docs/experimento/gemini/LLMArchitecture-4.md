...
This document defines the software architecture for the Google Drive-like system. This is the **Final Architecture**, incorporating secure storage, real-time synchronization, distributed scalability (sharding/replication), and collaboration features (sharing/ACLs).
...
*(Summary of critical drivers addressed across all iterations)*

  * **Functionality**: Upload (US-1.1), Download (US-1.3), Sync (US-2.1), Share (US-3.2), History (US-3.1).
  * **Reliability**: Data Durability (QAS-001), Disaster Recovery (QAS-002).
  * **Scalability**: 500 PB Data (QAS-008), 10M DAU (QAS-007).
  * **Security**: Encryption at Rest (QAS-016), Access Control (QAS-018).
  * **Efficiency**: Delta Sync (US-2.2), Compression (QAS-011).

## 4\. Views of the module viewtype

This view details the internal code structure and logic modules within the key servers.

### 4.1 Block Server Modules (The Optimization Pipeline)

```mermaid
classDiagram
    class BlockServer {
        +handleUpload(stream)
        +handleDownload(blockId)
    }
    class Chunker {
        +split(stream) : Chunk[]
    }
    ...
    BlockServer ..> Chunker
    BlockServer ..> Compressor
    BlockServer ..> Deduplicator
    BlockServer ..> Encryptor
```

### 4.2 API Server Modules (The Logic Layer)
...
classDiagram
    ...
    class AuthMiddleware {
        +verifyToken(token)
        +checkACL(user, file, permission)
    }
    class SyncEngine {
        +calculateDelta(clientHashes, dbHashes)
        +checkVersion(clientVer, dbVer)
    }
    class NotificationPublisher {
        +publishEvent(event)
    }

    APIServer ..> AuthMiddleware : Intercepts
    APIServer ..> SyncEngine
    APIServer ..> NotificationPublisher
```
...
### 5.4 Final System Topology

This view shows the complete distributed system, including the **Sharded Database**, **Replicated Storage**, **Notification System**, and **Background Services (GC)**.
...
graph TD
    subgraph Client_Layer
        Client["Client App<br/>(Sync Agent + Local DB)"]
    end

    subgraph Traffic_Entry
        LB["Load Balancer HA<br/>(SSL Termination)"]
    end

    subgraph Application_Cluster
        API["API Server Cluster<br/>(Auth, Sync, ACL)"]
        Block["Block Server Cluster<br/>(Compress, Encrypt, De-dup)"]
        Notif["Notification Service<br/>(Long Polling)"]
        Queue["Message Queue"]
    ...
    Block --> S3_Pri
    Block -- "Check Hash" --> DB_M
    
    GC -- "Scan Orphans" --> DB_S
    GC -- "Delete Objects" --> S3_Pri
```
...
| Unit | Technology Stack | Scale Strategy |
| :--- | :--- | :--- |
| **API Server** | Go/Java, REST | Stateless Horizontal Autoscaling (CPU-based). |
| **Block Server** | Go/C++, Streaming | Stateless Horizontal Autoscaling (Network-based). |
| **Metadata DB** | PostgreSQL/MySQL | **Sharded** by UserID, Master-Slave Replication. |
| **Notification** | Node.js/Go | Horizontal Scaling (Connection-based). |
| **Queue** | Kafka/RabbitMQ | Partitioned Cluster. |
| **Storage** | Amazon S3 | Managed Infinite Scale. |
...
### 7.4 Iteration 4 Drivers (Collaboration & Optimization)

#### US-3.2: File Sharing (ACL Update)
...
sequenceDiagram
    ...
    Owner->>API: POST /files/X/share { user: "Bob", role: "EDITOR" }
    
    Note over API: Auth Middleware Check
    API->>DB: Check if Owner has SHARE permission?
    DB-->>API: Yes
    
    Note over API: Business Logic
    API->>DB: INSERT INTO permissions (file, bob, EDITOR)
    API->>DB: UPDATE file SET version = version + 1
    
    API->>Notif: Publish "File Shared" to Bob
    API-->>Owner: 200 OK
```

#### US-1.1 + QAS-011 + De-dup: Optimized Upload Pipeline
...
sequenceDiagram
    ...
    Note over Block: 1. Compression
    Block->>Block: Compress (GZIP/ZSTD)
    
    Note over Block: 2. De-duplication Check
    Block->>Block: Calculate Hash (H1)
    Block->>DB: Exists(H1) in User Account?
    
    alt Hash Exists (Duplicate)
        DB-->>Block: Yes (RefID)
        Block->>Block: Skip Upload
    else Hash New
        DB-->>Block: No
        Note over Block: 3. Encryption
        Block->>Block: Encrypt (AES)
        Note over Block: 4. Storage
        Block->>S3: PutObject(H1_Encrypted)
    end
    
    Block->>DB: Add Block Reference to File
    Block-->>Client: 200 OK
```
...
### 8.5 Final Interface Catalogue

  * **I-01 to I-03**: Basic Upload/Download (See Iteration 1).
  * **I-04 to I-06**: Sync & Polling (See Iteration 2).
  * **I-07 to I-09**: Resumable Uploads (See Iteration 3).

#### I-10: File Sharing API

  * **Endpoint**: `POST /files/{id}/share`
  * **Input**: `{"target_email": "...", "role": "VIEWER|EDITOR"}`
  * **Output**: `200 OK`

#### I-11: Version History API

  * **Endpoint**: `GET /files/{id}/versions`
  * **Output**: `{"current": 10, "history": [{ver: 9, date: ...}, {ver: 8...}]}`
...
### 9.4 Final Consolidated Decision Log

| Area | Decision | Rationale |
| :--- | :--- | :--- |
| **Architecture Style** | **Service-Based / Distributed** | Separates Data Plane (Block Server) from Control Plane (API) to handle differing resource needs (CPU vs IO). |
| **Storage** | **Block-Based with De-dup** | Splitting files allows Delta Sync, Resumable Uploads, and De-duplication. |
| **Database** | **Sharded Relational (User ID)** | Sharding by User ID is the only way to hit 500PB while keeping "List Files" fast. Relational DB ensures ACID for metadata. |
| **Sync** | **Differential Sync + Optimistic Locking** | Minimizes bandwidth (Delta Sync) and prevents data corruption (First-write-wins). |
| **Security** | **ACL + Server-Side Encryption** | ACLs provide granular sharing. Server-side encryption ensures data is secure at rest without relying on client logic. |
| **Optimization** | **Account-Level De-dup** | Balances storage efficiency with user privacy (preventing side-channel leakage). |
| **Availability** | **Replication (DB + S3) + HA LB** | Removes all Single Points of Failure. |

***

**Architect's Conclusion**:
The design is now complete. We have successfully addressed the requirement to build a **secure, scalable, reliable, and collaborative** file storage system capable of serving **10M DAU** and storing **500PB** of data, as outlined in the Case Study.
